            ...
            if (ModelState.IsValid)
            {
                ...
                if (cart != null)
                {
                    ...
                    foreach (var sp in cart.Items)
                    {
                        ...
                        strSanPham += "<td>" + WebsiteBanGiayDep23.Common.Common.FormatNumber(sp.TotalPrice, 0) + "</td>";
                        strSanPham += "</tr>";
                    ...
                    contentCustomer = contentCustomer.Replace("{{ThanhTien}}", WebsiteBanGiayDep23.Common.Common.FormatNumber(thanhtien, 0));
                    contentCustomer = contentCustomer.Replace("{{TongTien}}", WebsiteBanGiayDep23.Common.Common.FormatNumber(TongTien, 0));
                    WebsiteBanGiayDep23.Common.Common.SendMail("HYPEBEAST SNEAKERS", "Đơn hàng #" + order.Code, contentCustomer.ToString(), req.Email);


                    //Gửi email cho admin để xác nhận đơn hàng
                    ...
                    contentAdmin = contentAdmin.Replace("{{ThanhTien}}", WebsiteBanGiayDep23.Common.Common.FormatNumber(thanhtien, 0));
                    contentAdmin = contentAdmin.Replace("{{TongTien}}", WebsiteBanGiayDep23.Common.Common.FormatNumber(TongTien, 0));
